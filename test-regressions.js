...
const { ROBINHOOD_CHAIN_CONFIG: config } = require('./packages/facilitator/config');
const { parseAmount } = require('./packages/facilitator/amount');
const { verifyPayment, claimMessage, EIP712_DOMAIN, EIP712_TYPES } = require('./packages/facilitator/verifier');
const { settlePayment } = require('./packages/facilitator/settler');
const vault = require('./packages/vault/storage');
...
test('production: simulated proofs remain disabled even when demo flag is set', () => {
  const child = spawnSync(process.execPath, ['-e', "const c=require('./packages/facilitator/config').ROBINHOOD_CHAIN_CONFIG;if(c.demoMode)process.exit(1)"], { cwd: __dirname, env: { ...process.env, NODE_ENV: 'production' } });
...
test('production: verification is read-only, settlement is durable and single use', async () => {
  ...
  const child = spawnSync(process.execPath, ['-e', "if(!require('./packages/facilitator/redemptions').isRedeemed(process.argv[1]))process.exit(1)", first.redemptionKey], { cwd: __dirname, env: process.env });
  assert.equal(child.status, 0, child.stderr.toString());
});
...
function browserFixture(provider, fetchImpl) {
  ...
  vm.runInContext(fs.readFileSync(path.join(__dirname, 'packages/public/app.js'), 'utf8'), context);

...
async function readBody(res) { const raw = await res.text(); try { return JSON.parse(raw); } catch (_) { return { raw }; } }
...
class AgentClient {
  ...
  async fetchWith402(path, options = {}) {
    ...
    if (challenge.scheme !== 'exact' || !ROBINHOOD_CHAIN_CONFIG.demoMode) throw new Error('This agent supports demo vouchers only; submit a confirmed transfer and payer claim for real payments');
    ...
    const resultData = await readBody(retryRes);
    ...
    steps.push({
      step: 5,
      phase: retryRes.ok ? 'DELIVERED' : 'PAYMENT_FAILED',
      description: `HTTP ${retryRes.status} received with receipt ${receipt?.receiptId || 'verified'}`,
      receipt
    });

...
router.get('/supported', (req, res) => {
  res.json({
    ...
    demoMode: ROBINHOOD_CHAIN_CONFIG.demoMode,
    testnet: ROBINHOOD_CHAIN_CONFIG.testnet,
    ...
    supportedNetworks: [
      {
        id: ROBINHOOD_CHAIN_CONFIG.networkId,
...
router.post('/verify', async (req, res) => {
  try {
    ...
    return res.json({
      ...
      network: ROBINHOOD_CHAIN_CONFIG.networkId,
      chainId: ROBINHOOD_CHAIN_CONFIG.chainId
    });
  } catch (err) {
...
router.post('/settle', async (req, res) => {
  try {
    ...
    const receipt = settlePayment(verifyResult, { ...metadata, endpoint: requirement.resource });
...
router.post('/merchants', (req, res) => {
  ...
  if (!recipient || !ethers.isAddress(recipient) || (endpoints && !Array.isArray(endpoints))) {

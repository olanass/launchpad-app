 ...
 * x402 Protocol encrypted storage metadata
 */
...
async function pinBufferToIpfs(buffer, filename = 'encrypted_payload.enc', meta = {}) {
  ...
  try {
    ...
    const pinataMetadata = JSON.stringify({
      ...
      keyvalues: {
        protocol: 'x402',
        network: `${config.networkId}-${config.chainId}`,
        cipher: 'aes-256-gcm',
        ...meta
      }
    });
...
async function fetchBufferFromIpfs(cid) {
  ...
  try {
    ...
    const res = await fetch(pinataUrl, {
      headers: {
        ...(process.env.PINATA_GATEWAY_TOKEN ? { 'x-pinata-gateway-token': process.env.PINATA_GATEWAY_TOKEN } : {})
      },
